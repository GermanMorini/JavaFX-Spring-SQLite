...
//    similar a la anotación @FXML, que inyecta los controles de la escena
    ...
    pagination.currentPageIndexProperty().addListener((obs, old, _new) -> {
        try {

        if (_new.intValue() == 1) {
            Scene direccionForm = FXMLLoader.load(getClass().getResource("direccion_form.fxml"));
        } else {
    ...
    @FXML
    private void paginationMC(MouseEvent me) {
